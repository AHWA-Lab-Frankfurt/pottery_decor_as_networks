...
map_bw_scale <- list(geom_sf(data = mali_adm0, fill = NA),
...
map_bw_grid <- list(geom_sf(data = mali_adm0, fill = NA),
                    geom_sf(data = rivers, fill = "grey", colour = "grey") ,
                    maptheme_bw_grid
)
...
inset <- ggplot()+
  ...
  geom_sf_text(data = mali_adm0, aes(label = admin), position = position_nudge(x = 15, y = 1), size = 3)+
  theme_void()

# 4. Composite colour map background

# import geotiff
colour_background <- rast(here::here("analysis/data/raw_data/ne_background.tif")) %>% mask(vect(mali_adm0))

#export to data frame, rename bands, and return to raster format
colour_background <- as.data.frame(colour_background, xy = TRUE) %>%
  rename(r = ne_background_1,
         g = ne_background_2,
         b = ne_background_3) %>%
  rast()

#this can now be plotted using terrainr's geom_spatial_rgb
color_map <- list(geom_spatial_rgb(data = colour_background, aes(x = x, y = y , r = red, g = green, b = blue)),
...
mountains <- dem.df %>%
  mutate(mountain = case_when(alt > 420 ~ 1,
                              TRUE ~ 0))

### Find a way to turn this into polygons and plot as geom_sf

bw.dem.map <- list(
  geom_raster(data = mountains,
