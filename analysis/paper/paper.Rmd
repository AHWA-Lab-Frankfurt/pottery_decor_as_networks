...
decor_18 <- extract_backbone(decor_flat, 0.18)
...
library(readr)
decorsearly <- 
  read_csv2(here::here("analysis/data/raw_data/decors_early.csv")) |>
  select(-1)
...
early_flat_bb <- early_flat_bb |>
  ...
  mutate(label = early_flat_nodes$label,
         lat = early_flat_nodes$lat, lon = early_flat_nodes$lon)
