...
library(iNEXT)
data("ciliates")
as.data.frame(decors) |> 
  column_to_rownames("site") |> 
  t() |> 
iNEXT(q=0, datatype="incidence_freq", size=NULL, endpoint=NULL, knots=40, se=TRUE, conf=0.95,
nboot=50)
...
# library(iNEXT.4steps)
 ...
 i <- as.data.frame(decors) |> 
  column_to_rownames("site") |> 
   head(3) |> 
  t() |> 
  iNEXT(q=0, datatype="abundance", size=NULL, endpoint=NULL, knots=40, se=TRUE, conf=0.95,
nboot=50)
...
i$summary$`STEP2. Asymptotic analysis`
       

i$summary$`STEP3. Non-asymptotic coverage-based rarefaction and extrapolation analysis`
       

i$summary$`STEP4. Evenness among species abundances`
...
# Custom plot combining rows and columns
plot(ca_result$ind$coord, type = "n", xlab = "Dimension 1", ylab = "Dimension 2", main = "CA Rows and Columns")

# Plot rows
points(ca_result$row$coord, col = "blue", pch = 16)

# Plot columns
points(ca_result$col$coord, col = "red", pch = 17)

# Add legend
legend("topright", legend = c("Rows", "Columns"), col = c("blue", "red"), pch = c(16, 17))
...
decor_graph <- decor_graph |>
  activate(edges) |> 
  filter(!name == "twisted_cord_roulette") |> 
  activate(nodes) |>
  mutate(louvain = as.character(group_optimal()))

decor_graph1 <- decor_graph |>
    activate(edges) |> 
   filter(str_detect(name, "roulette") | name %in% c("paint", "straight_mat_impression"))  |> 
  activate(nodes) |>
...
decor_graph1 <- decor_18 |>
  activate(nodes) |>
  mutate(louvain = as.character(group_louvain()))

decor_graph1 <- decor_graph |>
  activate(nodes) |>
  mutate(louvain = as.data.frame(decor_graph1, "vertices")$louvain)
...
decor_graph |>
  activate(edges) |> 
  filter(str_detect(name, "strip_roulette")| str_detect(name, "cord_roulette"))  |> 
  as.data.frame("edges") |> 
  group_by(name) |> 
  summarise()
```
...
communitycolor <-
  setNames(
    c("tomato", "cornflowerblue", "darkgreen", "yellow"),
...
ggraph(decor_graph1, x = lon, y = lat) +
...
decor_graph1 <- decor_graph1 |>
  add_community_edges()

# mean edge weights
mean_edge_weight <- decor_graph1 |>
...
g1 <- mean_edge_weight |>
  pivot_longer(!community, names_to = "key",
               values_to = "mean_edge_weight") |>
  ggplot(aes(x = community, y = mean_edge_weight)) +
...
centr <- pull_centralities_clust(decor_graph1)
...
decor_18 <- extract_backbone(decor_flat, 0.20) #20 ideale anzahl?

# plot
p1 <- plot.bb(decor_05)
p2 <- plot.bb(decor_15)
p3 <- plot.bb(decor_18) + geom_node_text(aes(label = label), repel = TRUE)
...
# extract backbone
early_flat_bb <- 
  backbone::disparity(as.igraph(early_flat), alpha = 0.25, narrative = TRUE) |>
