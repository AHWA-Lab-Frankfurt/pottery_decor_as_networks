...
Jj12plastic <- Jj12plastic |> 
  mutate(channel = x1+x2, #single and multiple grooves are treated as one category
...
Ounjg12 <- Ounjg12 |> 
  rename(braided_strip_roulette = "Fibre plate tressee simple ou alterne",
         ...
         cord = "Cordons" #this is described as "presénce d'un petit cordon sous un bord 
         )
...
Here we create two separate collated dataset for the 12th and 9th century networks by binding together the previously created data for each site and then further cleaning and standardising.
