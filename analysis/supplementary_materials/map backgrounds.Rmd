...
```{r}
africa_outline <- sf::st_read(here::here("analysis", "data", "raw_data", "Africa_outline.shp")) %>% sf::st_crop(xmax = 54, xmin = -20, ymax = 40, ymin = -36)
```
...
```{r subset adm0}
mali_adm0 <- ne_countries(scale = "medium", returnclass = "sf") %>% filter(admin == "Mali")
```
...
```{r}
rivers <- ne_download(scale = 50, type = "rivers_lake_centerlines", category = "physical", returnclass = "sf") %>% sf::st_intersection(mali_adm0) %>% filter (name == "Niger")
